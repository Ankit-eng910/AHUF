...
class OrderHistoryOutletCart(APIView):
    def get(self , request):
        ...
        orders = Order.objects.filter(
            ...
            cart_id__isnull=True,
            outlet_cart__isnull=False
        ).select_related('outlet_cart__booking').order_by('-created_at')


        serializer = OrderWithOutletCartItemsSerializer(orders , many=True, context={'request': request})

        return Response({
            "success": True,
            "message" : "Got It check in your terminal.",
            "data" : serializer.data
        })
